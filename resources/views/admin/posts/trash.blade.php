...
@section('content')
    <h2 class="mb-3">Danh Sách Bài Viết - Đang chờ xóa ({{ $trashCount }} bản ghi)</h2>
    {{-- gọi component --}}
    <x-admin.alert />
        ...
        <tbody>
            @foreach ($list as $item)
                <tr>
                    ...
                    <td>
                        @if ($item->image)
                            <img src="{{ asset('storage/posts/' . $item->image) }}" width="80" class="img-thumbnail">
                        @endif
                    </td>
                    <td>{{ $item->slug }}</td>
                    <td>{{ $item->title }}</td>
                    ...
                    <td>
                        <form action="{{ route('admin.posts.restore', $item->post_id) }}" method="POST" class="d-inline">

                            @csrf
                            @method('PATCH')
                            <button class="btn btn-success btn-sm">Khôi phục</button>
                        </form>
                        <form action="{{ route('admin.posts.forceDelete', $item->post_id) }}" method="POST"
                            class="d-inline">
